-- =========================================================================
-- SUPPORT OPTIONAL MOVIE IDS FILTER FOR AUTO-GENERATING SHOWTIMES (FIXED ENUM)
-- =========================================================================

DROP FUNCTION IF EXISTS generate_showtimes_for_next_days(INT);

CREATE OR REPLACE FUNCTION generate_showtimes_for_next_days(
  p_days_count INT,
  p_movie_ids BIGINT[] DEFAULT NULL
)
RETURNS void AS $$
DECLARE
  v_movie_ids BIGINT[];
...
BEGIN
  ...
  -- Xác định danh sách phim cần tạo lịch
  ...
  ELSE
    v_movie_ids := p_movie_ids;
  END IF;
  
  v_movies_count := array_length(v_movie_ids, 1);
  
  IF v_movies_count IS NULL OR v_movies_count = 0 THEN
    RETURN; -- Không có phim nào để tạo lịch
  END IF;

  -- 2. Tự động xóa các suất chiếu trong tương lai của các phim này mà chưa có ai đặt vé
  DELETE FROM showtimes
  WHERE start_time >= now()
    AND start_time <= date_trunc('day', now() AT TIME ZONE 'Asia/Ho_Chi_Minh') + (p_days_count || ' day')::INTERVAL
  ...
  -- 3. Sinh lịch chiếu cho p_days_count ngày tiếp theo
  FOR v_day_offset IN 0..(p_days_count - 1) LOOP
    ...
    FOR v_room IN SELECT * FROM rooms LOOP
      ...
      FOREACH v_time IN ARRAY v_times LOOP
        ...
        -- 4. Kiểm tra xem phòng chiếu này có bị trùng lịch chiếu khác trong khoảng thời gian này không
        IF NOT EXISTS (
          ...
          -- 5. Tính giá vé cơ bản dựa trên giờ chiếu và loại phòng
          ...
          -- 6. Thực hiện thêm suất chiếu mới
...
GRANT EXECUTE ON FUNCTION generate_showtimes_for_next_days(INT, BIGINT[]) TO authenticated;
