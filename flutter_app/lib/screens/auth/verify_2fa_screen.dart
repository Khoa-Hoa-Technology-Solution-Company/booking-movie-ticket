...
class _Verify2FAScreenState extends State<Verify2FAScreen> {
  ...
  Future<void> _handleResend() async {
    ...
    try {
      await authService.resend2FACode(email: widget.email);
      _startCooldownTimer();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Mã xác thực 2FA mới đã được gửi!'),
            backgroundColor: Colors.green,
          ),
        );
      }
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Nhập mã OTP vừa được gửi đến email để xác minh đăng nhập:\n${widget.email}',
